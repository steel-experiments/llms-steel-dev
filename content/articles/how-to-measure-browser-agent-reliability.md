...
| Plan-tier saturation | Steel Local is effectively one concurrent session while Steel Cloud Launch/Scale stretch into the tens or hundreds; once a tier hits 90 percent of its concurrency or RPM cap, startup p95 will spike before success drops. | Track `(active_sessions / plan_cap)` and `(requests_per_minute / plan_cap)` per plan tier; alert when either crosses 0.9 for 5 minutes. | Pricing/Limits table for caps + Sessions API `list` to count active sessions; annotate scoreboard rows with plan tiers. |
| Evidence coverage | If a failure lacks replay/logs/files, it cannot improve reliability. Evidence coverage should be 100 percent for failed runs. | `failures_with_replay / total_failures` plus storage lag for logs/files exports. | HLS replay URLs, MP4 exports, Files API attachments, `agent-traces` pagination. |
...
| Plan tier & warm pool size | Steel Local (~1 concurrent session) vs Steel Cloud (hundreds), plus Launch vs Scale caps, control how many sessions can boot before the queue saturates. | Join session metrics with plan metadata from Pricing/Limits and alert when any tier crosses 90% of its concurrency or RPM ceiling. |
| Proxy or fingerprint class | Headless vs headful, mobile mode vs desktop, or residential vs datacenter proxies fail in different ways. | Store your own `proxyPool`, `fingerprintMode`, and `mobileMode` labels in your metrics system (Steel exposes `useProxy`, `solveCaptcha`, `persistProfile`, `deviceConfig.device`, and `stealthConfig` on the session) and pivot success rates accordingly. |
...
| Plan tier | Concurrent sessions cap | Requests per minute cap | Reliability response |
| --- | --- | --- | --- |
...
| Enterprise (custom) | 1,000+ | Custom | Mirror the contracted ceiling and stop trusting “unlimited” without on-call automation; negotiate burst headroom so alerts stay meaningful. |
...
- Queues can drain minutes before a session ever boots when concurrency caps (10 on Launch, 100 on Scale, 1,000+ on Enterprise) are saturated.
...
3. **Log agent actions.** Call `GET /v1/sessions/:id/agent-traces` to align model steps, CDP commands, and human inputs with timestamps.
4. **Store evidence immediately.** Save HLS URLs, MP4 downloads, screenshots, and scraped files using the Files API so every failure has replayable proof even after the retention window closes (7 days on Launch, 14 days on Scale).
...
9. **Wire plan caps to alerts.** Pull concurrency and RPM ceilings straight from [Pricing/Limits](https://docs.steel.dev/overview/pricinglimits) and annotate every scoreboard row with the plan tier; alert early when startup p95 creeps upward while a plan is within 10 percent of saturation so ops can flip workloads from Steel Local (~1 concurrent session) to Steel Cloud (100+), or between Launch/Scale tiers, before prompts waste browser slots.
...
| First connect → first action | Agent Traces `events[].` entry ID / timestamp, action type (`goto`, selector click), DOM target, fingerprint mode (headful/mobile), proxy label | First-action success %, domain-by-domain anti-bot diagnosis |
...
| `region` | Compare startup and first-action health per data center before trusting blended numbers. | `lax`, `iad` |
| `planTier` | Watch for launch/scale plan saturation and know when to upgrade or shard workloads. | `launch`, `scale`, `enterprise` |
...
| `logsExportId` | Confirms Agent Traces finished exporting; without it you cannot align model actions to DOM state. | `log_exp_92ff` |
...
| `expiresAt` | Forces teams to pull artifacts before the retention window closes (7 days Launch / 14 days Scale). | `2026-04-14T00:00:00Z` |
...
| Plan-tier saturation | <90% of concurrency or RPM cap per tier | ≥90% for 5 consecutive minutes | Control-plane + growth owners: shift jobs off Steel Local (~1 session) onto Steel Cloud (100+), or move Launch workloads to Scale/Enterprise tiers before queues stall. |
...
| Plan tier ≥90% of its concurrency or RPM cap | 5-minute rolling window per plan tier | Auto-scale the pool or shift workloads to the next Steel Cloud tier before saturation drags startup p95 above the benchmark, then notify owners to rebalance queue fan-out. |
...
2. Pull the top three manual intervention reason codes and pin their replays plus Agent Traces to the bug tracker before the next queue resumes.
...
| Plan-cap check | Daily | Which plan tiers or warm pools exceeded 90% of concurrency/RPM? | Action list to shift jobs between Steel Local vs Cloud or across Launch/Scale tiers. |
...
| Agent decisions | `GET /v1/sessions/:id/agent-traces` | Returns the Agent Traces timeline (`events` array with `type`, `timestamp`, `target`, page context) to align prompts, actions, and DOM results for both automation and audit trails. |
...
- Steel Cloud session timeouts are plan-tiered — 15 minutes on Launch, 1 hour on Scale, and 24 hours on Enterprise; jobs longer than your tier's ceiling must checkpoint progress into Files API or storage and resume in a fresh session.
- Session replays, logs, and artifacts are retained for 7 days on Launch and 14 days on Scale (Enterprise is custom). Export anything compliance-sensitive before your plan's window closes.
...
- Not yet for workflows that routinely exceed your plan's session timeout (15 min / 1 hr / 24 hr for Launch / Scale / Enterprise) or run disconnected from the network; you will lose telemetry before the run ends.

## Next steps
- Wire your metrics sink to the [Pricing and Limits](https://docs.steel.dev/overview/pricinglimits) table so alarms fire before you exceed concurrency or RPM caps.
